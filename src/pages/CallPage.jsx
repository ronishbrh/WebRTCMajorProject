...
// Must match the key format used in HomePage / UserContext
...
export default function CallPage() {
	...
	useEffect(() => {
		...
		const init = async () => {
			...
			console.log("[CallPage] Socket found, isOpen:", wsRef.current.isOpen());

			// 6. Subscribe to signalling messages
			unsubscribers.push(wsRef.current.subscribe("join", async () => {
			...
			unsubscribers.push(wsRef.current.subscribe("challenge1", async (data) => {
				...
				if (!valid) { console.error("Unverified signature in challenge1"); return; }
			...
			unsubscribers.push(wsRef.current.subscribe("challenge2", async (data) => {
				...
				const valid = await verifyChallenge(contactPubKey, rawECDH, signature);
				if (!valid) { console.error("Unverified signature in challenge2"); return; }
			...
			unsubscribers.push(wsRef.current.subscribe("offer", async (data) => {
				...
				for (const c of pendingCandidates.current) {
					try { await pc.addIceCandidate(c); } catch (e) { console.error("ICE error", e); }
				}
			...
			unsubscribers.push(wsRef.current.subscribe("answer", async (data) => {
			...
			unsubscribers.push(wsRef.current.subscribe("ice", async (data) => {
				if (!AESKey.current) {
			...
			unsubscribers.push(wsRef.current.subscribe("end-call", () => {
			...
			// 7. Start handshake
			// Delay gives callee's CallPage time to mount and subscribe to "join"
			// before we send it — otherwise join arrives before handlers are ready
