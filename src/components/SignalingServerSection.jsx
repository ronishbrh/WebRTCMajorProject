...
export default function SignalingServerSection() {
    ...
    const requestAccess = async (serverURL) => {
        try {
            ...
            const res = await fetch(`${httpURL}/register`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ publicKey: pubKeyBase64, message: `Requesting access to server: ${serverURL}` })
            });
            if (!res.ok) throw new Error("Registration failed");

            const token = await authenticateWithServer(httpURL, identity.publicKey, identity.privateKey);
    ...
    const revokeAccess = async (serverURL) => {
        ...
        try {
            ...
            if (!token) {
                token = await authenticateWithServer(httpURL, identity.publicKey, identity.privateKey);
        ...
        const poll = async () => {
            for (const server of servers) {
                ...
                if (currentStatus === "approved") {
                    ...
                    try {
                        const token = await authenticateWithServer(toHttp(server.url), identity.publicKey, identity.privateKey);
                        if (token) {
    ...
    useEffect(() => {
        ...
        identityManager._getObject("keys", identity.userName)
            .then(r => setActiveServer(r?.activeSignallingServer || null))
            .catch(console.error);
    }, [identity?.userName]);

    const handleSetActive = async (serverURL) => {
        try { await identityManager.setActiveSignallingServer(identity.userName, serverURL); setActiveServer(serverURL); }
        catch { alert('Failed to set active server'); }
    };
    ...
    const handleDeleteServer = async (serverURL) => {
        ...
        try {
            ...
            for (const c of contacts) {
                if (c.signalingServerURL === serverURL)
                    await identityManager.updateContactSignalingServer(identity.userName, c.userName, null);
            }
        ...
        <div className="space-y-4">
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={handleAddServer} disabled={addingServer || !newServerURL.trim()}
                                    ...
                                    <div key={server.url}
                                        ...
                                        <div className="flex items-start justify-between gap-3 flex-wrap">
                                            ...
                                            <div className="flex gap-2 flex-shrink-0 items-start flex-wrap">
                                                {!isActive && (
                                                    <button onClick={() => handleSetActive(server.url)}
                                                        className="p-2 bg-blue-600 hover:bg-blue-700 text-white rounded transition" title="Set as active server">
                                                        <FiCheck size={16} />
                                                    </button>
                                                )}
